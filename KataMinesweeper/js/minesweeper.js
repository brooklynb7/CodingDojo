	...
	for (var r = 0; r < field.length; r++) {
		for (var c = r; c < col; c++) {
			for (var i = 0; i < mines.length; i++) {
				if (mines[i].x === r && mines[i].y === c) field[r][c] = '*';
			}
		}
	}
	return field;
...
function generateMineSweeper(row, col, mines) {
	...
	for (var mine = 0; mine < mines.length; mine++) {
		if (mines[mine].x >= 0 && mines[mine].x < row && mines[mine].y >= 0 && mines[mine].y < col) {
			for (var l = mines[mine].x - 1 <= 0 ? 0 : mines[mine].x - 1;
			(l <= mines[mine].x + 1 && l < row); l++) {
				for (var c = mines[mine].y - 1 <= 0 ? 0 : mines[mine].y - 1;
				(c <= mines[mine].y + 1 && c < col); c++) {
